...
"Bidder 1 name","Bidder 1 country", "Bidder 2 name","Bidder 2 country", "Bidder 3 name","Bidder 3 country", "Bidder rest name","Bidder rest country","Contract number",
...
while True:
  url = "https://x.x.x.x:5567/protected/clean_tender/timestamp/2016-07-01T12:30:00.000/source/eu.dfid.worker.wb.clean.WBContractCleaner/page/%s?auth_token=%s" % (page, token)
    ...
    for data in result_set:
      ...
      if lots is not None and len(lots) > 0:
        ...
        if bids is not None and len(bids) > 0:
          bidder1Name = getWinnerName(bids[0])
          bidder1Country = getWinnerCountry(bids[0])
        if bids is not None and len(bids) > 1:
          bidder2Name = getWinnerName(bids[1])
          bidder2Country = getWinnerCountry(bids[1])
        if bids is not None and len(bids) > 2:
          bidder3Name = getWinnerName(bids[2])
          bidder3Country = getWinnerCountry(bids[2])
        if bids is not None and len(bids) > 3:
          index = 0
          for bid in bids:
            if index > 2:
              if bidderRestName is None:
                bidderRestName = getWinnerName(bids[index])
                bidderRestCountry = getWinnerCountry(bids[index])
              else:
                bidderRestName =  bidderRestName + ";" + getWinnerName(bids[index])
                bidderRestCountry =  bidderRestCountry + ";" + getWinnerCountry(bids[index])
            index += 1
      ...
      row.append(bidder1Name);
      row.append(bidder1Country);
      row.append(bidder2Name);
      row.append(bidder2Country);
      row.append(bidder3Name);
      row.append(bidder3Country);
      row.append(bidderRestName);
      row.append(bidderRestCountry);
...
with open('c:\\tmp\\wb_tender_contracts.csv', 'w', newline='', encoding='utf-8') as f:
  writer = csv.writer(f)
