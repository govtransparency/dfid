...
"Bid deadline","Buyer assigned ID","Contract signature date","Description","Final Price","Fiscal year",
...
while True:
  url = "https://x.x.x.x:5567/protected/clean_tender/timestamp/2010-01-01T12:30:00.000/source/eu.dfid.worker.wb.clean.WBMajorContractAwardsSODACleaner/page/%s?auth_token=%s" % (page, token)
  ...
  if(response.ok):
    ...
    for data in result_set:
      index = index+1
      row=[]
      projectBorrowerCountry = data.get("project").get("borrower").get("address").get("country") if data.get("project").get("borrower") is not None and data.get("project").get("borrower").get("address") is not None else None
...
with open('c:\\tmp\\wb_tender_majorCA.csv', 'w', newline='', encoding='utf-8') as f:
  writer = csv.writer(f)
