...
def processFinancingSummaries(projectId, summaries):
  for summary in summaries:
    ...
    summaryCancelledAmount = summary.get("cancelledAmount").get("netAmount") if summary.get("cancelledAmount") is not None else None    
...
header=["Project ID","Project approval date","Project borrower financing","Project country","Project description",
      "Project donor financing","Project final cost","Project status","Project signature dates","Major sectors","Sectors",
      ...
      "Publication source form type","Publication form type", "Financing summary disbursed", "Financing summary repayments", "Financing summary cancelled amount"]
rows.append(header)
financingSummaryHeader=["Project ID","Disbursed","Repayments"]
financingSummaries.append(financingSummaryHeader)

url = "https://78.46.99.116:5567/login"
...
while True:
  url = "https://78.46.99.116:5567/protected/clean_project/timestamp/2016-07-01T12:30:00.000/source/eu.dfid.worker.idb.clean.IDBProjectDetailsCleaner/page/%s?auth_token=%s" % (page, token)
  ...
  if(response.ok):
    ...
    for data in result_set:
      ...
      financingSummary = data.get("financingSummary")
      summaryDisbursed = None
      summaryRepaymens = None
      summaryCancelledAmount = None
      if financingSummary is not None and len(financingSummary) > 0:  
        summaryDisbursed = financingSummary[0].get("disbursed").get("netAmount") if financingSummary[0].get("disbursed") is not None else None
        summaryRepaymens = financingSummary[0].get("repayments").get("netAmount") if financingSummary[0].get("repayments") is not None else None
        summaryCancelledAmount = financingSummary[0].get("cancelledAmount").get("netAmount") if financingSummary[0].get("cancelledAmount") is not None else None
      ...
      if tenderMajorSectors is not None:
        for majorSector in tenderMajorSectors:
          if msColumnValue is None:
            if majorSector.get("name") is not None:
              msColumnValue = majorSector.get("name")
          else:
            if majorSector.get("name") is not None:
              msColumnValue = msColumnValue + ";" + majorSector.get("name")
      ...
      if tenderSectors is not None:
        for sector in tenderSectors:
          if sColumnValue is None:
            if sector.get("name") is not None:
              sColumnValue = sector.get("name")
          else:
            if sector.get("name") is not None:
              sColumnValue = sColumnValue + ";" + sector.get("name")
      ...
      row.append(summaryDisbursed)
      row.append(summaryRepaymens)
      row.append(summaryCancelledAmount)
...
with open('c:\\tmp\\idb_project_details.csv', 'w', newline='', encoding='utf-8') as f:
  writer = csv.writer(f)
  writer.writerows(rows)
  
with open('c:\\tmp\\idb_project_financing_summaries.csv', 'w', newline='', encoding='utf-8') as f:
  writer = csv.writer(f)
